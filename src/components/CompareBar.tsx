...
export function CompareBar({ variants, onOpenCompare, onRemove }: CompareBarProps) {
      ...
      <div className="mx-auto flex max-w-3xl flex-col gap-3 rounded-2xl border border-ink-100 bg-white px-4 py-3 shadow-[0_8px_30px_rgba(20,10,30,0.12)] sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2 overflow-x-auto">
          {variants.map((v) => (
            ...
            >
              {v.price} DH · {variantDisplayName(v)}
              <button
                type="button"
                onClick={() => onRemove(v.id)}
                aria-label={`Retirer ${variantDisplayName(v)} ${v.price} DH du comparateur`}
        ...
        <div className="flex shrink-0 items-center justify-between gap-4 sm:justify-end">
          <p className="text-sm font-medium text-ink-500">{variants.length} forfaits sélectionnés</p>
          <button
            type="button"
